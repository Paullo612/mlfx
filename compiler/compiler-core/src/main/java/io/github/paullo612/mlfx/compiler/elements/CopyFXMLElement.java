...
public class CopyFXMLElement extends IdentifiableFXMLElement {

    private TypedContinuation continuation;

    public CopyFXMLElement(FXMLElement<?> parent) {
        super(parent);
    }

    @Override
    public void handleAttributeLookahead(CompilerContext context, String prefix, String localName, String value) {
        if ((prefix == null || prefix.isEmpty()) && FXMLLoader.COPY_SOURCE_ATTRIBUTE.equals(localName)) {
    ...
    public void handleAttributesLookaheadFinish(CompilerContext context) {
        if (continuation == null) {
            throw context.compileError(FXMLLoader.COPY_SOURCE_ATTRIBUTE + " attribute is required.");
        }

        ClassElement classElement = getClassElement();

        // Search for copy constructor
        MethodElement copyConstructor = classElement.getEnclosedElement(ElementQuery.CONSTRUCTORS
                        .onlyAccessible()
                        .filter(m -> m.getParameters().length == 1)
                        .filter(m -> ElementUtils.isAssignable(classElement, m.getParameters()[0].getType()))
                )
                .orElseThrow(() ->
                        context.compileError(
                                "No copy constructor found for " + ElementUtils.getSimpleName(classElement) + "."
                        )
                );

        ExpressionContext.RenderCommand renderCommand = continuation.run();

        // Render copy constructor and save constructed object to local variable.
        context.getRenderer().render(methodVisitor -> {
            Type objectType = RenderUtils.type(classElement);
            Type argumentType = RenderUtils.type(copyConstructor.getParameters()[0].getType());

            methodVisitor.newInstance(objectType);
            methodVisitor.dup();
            renderCommand.render(methodVisitor);
            methodVisitor.invokeConstructor(
                    objectType, new Method(RenderUtils.CONSTRUCTOR_N, "(" + argumentType + ")V")
            );
    ...
    public CompilerContext.RenderCommand load() {
        // NB: Primitives do not have copy constructor, so, value is always stored in local variable.
